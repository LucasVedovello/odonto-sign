...
import { Loader2, Stethoscope, CheckCircle2, ArrowLeft, ArrowRight, FileText } from "lucide-react";
...
function CreateCompanyPage() {
  ...
  // ── Submit ──────────────────────────────────────────────────────
  const submit = async () => {
    // Revalida todas as etapas anteriores antes de enviar
    for (let s = 0; s <= 4; s++) {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        data: {
          // company_name mantém compatibilidade com o restante do app
    ...
    // Notifica o admin (edge function resolve o companyId via service role)
    try {
      const { error: fnError } = await supabase.functions.invoke("notify-new-company", {
        body: {
          companyName: form.nome_fantasia.trim() || form.razao_social.trim(),
          ownerName: form.responsavel_nome.trim(),
          ownerEmail: form.email.trim(),
          createdAt: new Date().toISOString(),
        },
      });
      if (import.meta.env.DEV && fnError)
        console.error("[criar-empresa] notify-new-company error:", fnError);
    } catch (e) {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center px-4">
        <Card className="max-w-md p-8 text-center">
          ...
          <h2 className="mt-4 text-xl font-semibold">Cadastro realizado!</h2>
              ...
              <>
                <div className="rounded-lg border bg-muted/40 p-4">
                  ...
                  <p className="text-sm text-muted-foreground">
                    A OdontoSign é uma plataforma SaaS de gestão para clínicas odontológicas. Ao se
                    cadastrar, você concorda com as regras de uso, o fluxo de aprovação manual da
                    conta e o tratamento de dados pessoais conforme a{" "}
                    <strong>LGPD (Lei nº 13.709/2018)</strong>. O cadastro fica sujeito à aprovação
                    do administrador antes da liberação do acesso.
                  </p>
                  <Button
                    type="button"
                    variant="link"
